    ...
    for korisnik in korisnici:
        if korisnik["korisnicko_ime"] == korisnicko_ime:
            if korisnik["lozinka_hash"] == hash_data(lozinka):
                return web.json_response({"Poruka" : "Korisnik uspješno prijavljen.", "auth_ok":True}, status=200)
            return web.json_response({"Poruka": "Lozinka neispravna.", "auth_ok": False} ,status=403)
    return web.json_response({"Poruka": "Korisnik ne postoji u bazi.", "auth_ok": False} ,status=403)
